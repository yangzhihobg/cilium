	...
	if policyImportErr == nil && len(rules) > 0 {
		...
		if policyImportErr == nil {
			...
			rev, policyImportErr = d.PolicyAdd(rules, &AddOptions{
				ReplaceWithLabels: k8sUtils.GetPolicyLabels(
					cnp.ObjectMeta.Namespace,
					cnp.ObjectMeta.Name,
					k8sUtils.ResourceTypeCiliumNetworkPolicy),
			})
		}
	}
...
func (d *Daemon) deleteCiliumNetworkPolicyV2(cnp *cilium_v2.CiliumNetworkPolicy) {
	...
	labels := k8sUtils.GetPolicyLabels(cnp.ObjectMeta.Namespace, cnp.ObjectMeta.Name,
		k8sUtils.ResourceTypeCiliumNetworkPolicy)
...
func (d *Daemon) updateCiliumNetworkPolicyV2(ciliumV2Store cache.Store,
	...
	if len(newRules) == 0 && len(oldRules) != 0 {
		lbls := k8sUtils.GetPolicyLabels(oldRuleCpy.ObjectMeta.Namespace, oldRuleCpy.ObjectMeta.Name,
			k8sUtils.ResourceTypeCiliumNetworkPolicy)
		d.PolicyDelete(lbls)
	}
